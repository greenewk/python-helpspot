...
class HelpSpotAPI:
    ...
    def __init__(self, method, user, password, uri):
        ...
        # self.authz = base64.b64encode(s) # probably gonna nix this
    ...
    def __call__(self, **kwargs):
        ...
        req = requests.get(uri, auth=(self.user, self.password))
        return req.json()
        # Older Python versions may want to uncomment the following line
        # to prevent the server from keeping the connection open.
        # Below is legacy auth code before implementing Python3 and requests library
...
class HelpSpot:
    ...
    def __init__(self, uri, user, password, debuglevel=0):
        ...
        self.user = user
        self.password = password
        opener = build_opener(HelpSpotHandler(debuglevel=debuglevel))
